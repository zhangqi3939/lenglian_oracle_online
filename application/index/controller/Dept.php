...
class Dept extends Controller
{
    ...
    public function dept_save()
    {
        ...
        if($id > 0){
            $dept_exit = Db::name('department')->where('ID',$id)->find();
            if($dept_exit['TITLE'] !== $d['TITLE']){
                $exit = Db::name('department')->where('TITLE',$d['TITLE'])->find();
                if(!empty($exit)){
                    app_send_400('该部门已存在，请仔细核对部门信息再提交');
                }
            }
            Db::name('department')->where('id',$id)->update($d);
        }else{
            $dept_exit = Db::name('department')->where('TITLE',$d['TITLE'])->find();
            if(!empty($dept_exit)){
                app_send_400('该部门已存在，请仔细核对后再提交');
            }
    ...
    public function user_listing(){
            $user = Db::name('user')->field("ID,USER_NAME,REAL_NAME,TEL,EMAIL,GENDER,BRANCH_ID")->order('ID DESC')->select();
    ...
    public function user_save()
    {
        ...
        //查询提交用户是否存在
        if(empty($id)){
            $user_exit = Db::name('user')->where('USER_NAME',$formData['USER_NAME'])->find();
            if(!empty($user_exit)){
                app_send_400('该用户已存在，请仔细核对用户信息再提交');
            }
            ...
            $user_exit = Db::name('user')->where('ID',$id)->find();
            if($user_exit['USER_NAME'] !== $formData['USER_NAME']){
                $exit = Db::name('user')->where('USER_NAME',$formData['USER_NAME'])->find();
                if(!empty($exit)){
                    app_send_400('该用户已存在，请仔细核对用户信息再提交');
                }
            }
    ...
    public function role_save()
    {
        ...
        if(empty($role_id)){
            ...
            $role_exit = Db::name('rbac_role')->where('ID',$role_id)->find();
            if($role_exit['ROLE_NAME'] !== $d['ROLE_NAME']){
                $exit = Db::name('rbac_role')->where('ROLE_NAME',$d['ROLE_NAME'])->find();
                if(!empty($exit)){
                    app_send_400('该角色已存在，请仔细核对角色信息再提交');
                }
            }
    ...
    {
        $result = Db::name('rbac_limit')->field('ID,LIMIT_NAME')->order('ID ASC')->select();
        app_send(arraykeyToLower($result));
    }
    //角色分组【列表】
